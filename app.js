...
function applyTenantTheme(theme) {
    ...
    // 03.09. Перерисовываем оформление главной ЗДЕСЬ, а не только при старте:
    // подпись под баннером берётся из настроек тенанта, а они приходят позже,
    // чем первая отрисовка. Из-за этого у Анны подпись была задана, но на
    // экране её не было — элемент оставался пустым и скрытым.
    try { if (typeof _renderHomeShell === 'function') _renderHomeShell(null); } catch (e) {}
}
