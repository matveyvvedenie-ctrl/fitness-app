...
// Словарь синонимов: для каждой мышцы — ключевые слова, по которым можно опознать упражнение по названию.
// Используется когда в листе «Упражнения» не заполнена колонка «Группа мышц».
var MUSCLE_KEYWORDS = {
    'ноги':       ['ног', 'присед', 'выпад', 'жим ног', 'разгибан', 'сгибан ног', 'сгибан лёж', 'сгибан лежа', 'румынск', 'болгарск', 'квадрицепс', 'бедра', 'бедер', 'голень', 'ягодич', 'gakk', 'гакк', 'мертв'],
    'грудь':     ['жим лёж', 'жим лежа', 'жим лёж', 'жим штанги лёж', 'жим штанги лежа', 'разводк', 'разведен', 'кроссовер', 'бабочк', 'отжим', 'жим в смите', 'жим гантел', 'жим в хаммер', 'верх груд', 'пуловер'],
    'спина':     ['тяга', 'подтягив', 'шраг', 'гиперэкстенз', 'верхн блок', 'верхний блок', 'нижн блок', 'нижний блок', 'к поясу', 'к груд', 'к подбород', 'становая', 'мертв'],
    'плечи':     ['жим над', 'жим стоя', 'жим вверх', 'жим арнольд', 'мах', 'махов', 'арнольд', 'свинг', 'дельт', 'подъём перед', 'подъем перед', 'подъём в сторон', 'подъем в сторон'],
    'бицепс':    ['бицепс', 'сгибан рук', 'сгибание рук', 'молот', 'сгибание на бицепс', 'подъём на бицепс', 'подъем на бицепс', 'curl', 'скам'],
    'трицепс':   ['трицепс', 'разгибан рук', 'разгибание рук', 'французск', 'жим узк', 'отжим на брус', 'pushdown', 'каб трицепс'],
    'ягодицы':   ['ягодич', 'ягод', 'гиперэкстенз', 'мостик', 'отведен ног', 'румынск', 'болгарск', 'хип траст', 'thrust'],
    'пресс':     ['пресс', 'скруч', 'планк', 'подъём ног', 'подъем ног', 'дровосек'],
    'икры':      ['икр', 'икроножн', 'подъём на носк', 'подъем на носк'],
    'предплечья': ['предплеч', 'кист', 'wrist']
};

// Проверяет, относится ли упражнение к указанной мышце.
// Сначала смотрит на поле group (если заполнено), потом ищет ключевые слова в названии.
function exerciseHitsMuscle(ex, muscle) {
    var g = (ex.group || '').toLowerCase();
    var n = (ex.name || '').toLowerCase();
    if (g && g.indexOf(muscle) >= 0) return true;
    // Прямое вхождение названия мышцы в имя упражнения
    if (n.indexOf(muscle) >= 0) return true;
